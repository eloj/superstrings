...
Finding the _shortest_ such superstring is NP-hard but [APX-complete](https://en.wikipedia.org/wiki/APX), so there are algorithms to approximate it.
...
```console
usage: superstring [-h] [-q | -v] [-s | -S | -L LOOPS | -B | -A ALGO] [-C STR] [-F STR] [-j] [-i] [-l] [-R] [-G] [-P N] [-M N] [-V] [infile]
...
options:
  ...
  -P N, --partitions N     Partition input into subproblems of size N
  -M N, --mp N             Spawn N processes, only valid with -P
...
For large inputs, the option `--partitions N` can be used. This will partition the input into subproblems of N strings, and repeatedly process
groups with the chosen algorithm until a final superstring is reached. Using this option should be a 'last resort', as the result will be _much_
less optimal. You're almost guaranteed to be better off using `greedy` on the full input rather than partitioning and using `--brute`.

Combine `--partitions` with `--mp N` to process subproblems using N workers in parallel.

## Python Superstring Library API

`generate_superstring(list, func=greedy) -> str`
: Given a list of strings, returns a superstring. Default algorithm is 'greedy'. Invokes `make_substring_free` first.
